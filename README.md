...
Windows タスクスケジューラに、**回復までのカウントダウンを始めるための軽いリクエスト（kick）** を登録し、指定した回復時刻と曜日に合わせて **5 時間前から** 自動で再試行します。

## 概要

このツールは、**回復していてほしい時刻** と **実行曜日** を指定して、それに対応する kick を自動登録するためのものです。
...
指定した時刻に対応する kick は、その **5 時間前** を起点に **30 分おきで 2 時間** 再試行する形で組み立てます。ここでいう kick は、新しい 5 時間ウィンドウの開始を狙う軽いリクエストです。
...
最初は、**平日** に 2 〜 3 個の回復時刻を置く設定から始めるのが扱いやすいです。
...
デフォルトでは **月〜金** にチェックが入っています。

回復時刻は、利用したい時間帯に合わせて追加してください。
...
フォームではこの競合や曜日未選択をその場で警告します。  
近すぎる時刻がある場合、タスク登録は行いません。

## 仕組み

1. ユーザーが **回復していてほしい時刻** と **曜日** を選択
2. ツールがその **5 時間前** を起点に、**30 分おきに 2 時間** の再試行 kick を計算
3. Windows スケジュールタスク (`AgentTimerKick-Claude` / `AgentTimerKick-Codex`) を週次トリガーで登録
4. 指定時刻になると、回復までのカウントダウン開始を狙って次のコマンドを実行
...
- **曜日**  
  実行したい曜日を選びます。デフォルトは平日です
- **プレビュー**  
  `曜日: 月 火 水 木 金` と `11:10-13:10 kick /30m -> 16:10` のように表示します
- **WSL**  
...
**Q: 回復時刻はいくつ入れるべき？**  
A: まずは平日の `11:00 / 16:10 / 21:20` のような 2 〜 3 個から始めるのがおすすめです。近すぎる時刻を増やしても両立しません。

**Q: `kick` とは何？**  
A: この README では、**新しい 5 時間ウィンドウの開始を狙って送る軽いリクエスト** を `kick` と呼んでいます。
...
**Q: 土日は止められる？**  
A: はい。曜日ごとにチェックできます。デフォルトは平日だけ有効です。
